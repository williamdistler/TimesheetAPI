...
public class Relatorio {
    ...
    public static String geraRelatorio(List<Valores> lista) {
        ...
        resultado = funcionario.stream().map((func) -> func + ": " + Utils.decimalFormat(Utils.somarHorasFuncionario(lista, func)/60.0) + " horas trabalhadas no total" + System.lineSeparator()).reduce(resultado, String::concat);

        resultado += System.lineSeparator() + "---------Projetos--------" + System.lineSeparator();
        resultado = projeto.stream().map((proj) -> proj + ": " + Utils.decimalFormat(Utils.somarHorasProjeto(lista, proj)/60.0) + " horas de projeto no total" + System.lineSeparator()).reduce(resultado, String::concat);
